...
class UnitedStates:
    ...
    def retrieve_city(self, name):
        ...
        for city in self.cities:
            ...
            if city.city_name == name:
                return city
        print("New city: ", name)
        if name == "Los Angeles, California, USA":
            city = City(name, 1)
    ...
    def plot_cities(self):
        ...
        for city in self.cities:
            ...
            if city.city_name == "Boston, Massachusetts, USA":
    ...
    def plot_edges(self):
        ...
        for u, v, weight in self.network.edges(data='weight'):
            if weight is not None:
                ...
                plt.plot([lon, lon2], [lat, lat2], color='red', linewidth=1, marker='o',transform=ccrs.Geodetic(),)
            pass
    ...
    def plot_infections(self, ax):
        ...
        for city in self.cities:
            lat, lon = geoscrape(city)
            if city.city_name == "Boston, Massachusetts, USA":
                plot = plt.text(lon + 2 , lat + 3.5, str(city.number_infected), color ='red', 
    ...
    def plot_infection_circle(self, ax, lat, lon, num_inf):
        ...
        circle_points = cartopy.geodesic.Geodesic().circle(lon=lon, lat=lat, radius=(math.sqrt(num_inf) * 1500), endpoint=False)
